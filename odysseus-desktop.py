...
def main() -> None:
    ...
    # Give the process its own AppUserModelID so the taskbar groups it under
    # Odysseus (with odysseus.ico) instead of the generic Python host icon.
    # Windows reads this when the taskbar button is created, so it must be set
    # before any window exists — hence here, before the webview is started.
    if sys.platform == "win32":
        try:
            import ctypes
            ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(
                "Odysseus.AIWorkspace"
            )
        except Exception as e:
